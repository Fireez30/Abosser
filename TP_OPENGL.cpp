...
void displayVoxelRecCylinder(Voxel* v,CPoint* c,CVector vc,double r, double resolution,double cpt){
    if (cpt < resolution)
    {
        for (int i=0;i<8;i++)
            {
                Voxel* tmp = Voxel(&v->getSubCentersPoints()->at(i),(double)v->getLength()/2.0);
                int test = v->isInsideCylinder(c,vc,r);
                ...
                else
                {
                    if (cpt == resolution -1)
                        ...
                        displayVoxelRecCylinder(tmp,c,vc,r,resolution,cpt+1);
                }
            }
    }
}
/* */
void DisplaySOUSTRACTIONSphereCylinder(Voxel v,CPoint csphere,double rsphere,CPoint ccylinder,CVector vc,double rcylinder,double resolution){
